...
func NewFailureModule() Module {
	return FailureModule{key: "failure", priority: 38, family: "asset", focus: "failure mode classification"}
}

func (m FailureModule) Key() string {
	return m.key
}

func (m FailureModule) Description() string {
	return m.focus
}

func (m FailureModule) Priority() int {
	return m.priority
}

func (m FailureModule) Family() string {
	return m.family
}

func (m FailureModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m FailureModule) Validate(input string) error {
...
func (m FailureModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m FailureModule) Transition(stage string) bool {
...
func (m FailureModule) Evidence(input string, now time.Time) Evidence {
...
func (m FailureModule) Score(input string) int {
	value := strings.TrimSpace(input)
