...
func NewRecoveryModule() Module {
	return RecoveryModule{key: "recovery", priority: 62, family: "asset", focus: "recovery readiness evidence"}
}

func (m RecoveryModule) Key() string {
	return m.key
}

func (m RecoveryModule) Description() string {
	return m.focus
}

func (m RecoveryModule) Priority() int {
	return m.priority
}

func (m RecoveryModule) Family() string {
	return m.family
}

func (m RecoveryModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m RecoveryModule) Validate(input string) error {
...
func (m RecoveryModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m RecoveryModule) Transition(stage string) bool {
...
func (m RecoveryModule) Evidence(input string, now time.Time) Evidence {
...
func (m RecoveryModule) Score(input string) int {
	value := strings.TrimSpace(input)
