...
func NewRiskModule() Module {
	return RiskModule{key: "risk", priority: 19, family: "asset", focus: "risk posture calculation"}
}

func (m RiskModule) Key() string {
	return m.key
}

func (m RiskModule) Description() string {
	return m.focus
}

func (m RiskModule) Priority() int {
	return m.priority
}

func (m RiskModule) Family() string {
	return m.family
}
...
func (m RiskModule) Validate(input string) error {
...
func (m RiskModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m RiskModule) Transition(stage string) bool {
...
func (m RiskModule) Evidence(input string, now time.Time) Evidence {
...
func (m RiskModule) Score(input string) int {
	value := strings.TrimSpace(input)
