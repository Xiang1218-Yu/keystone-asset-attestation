...
func NewSerialModule() Module {
	return SerialModule{key: "serial", priority: 14, family: "asset", focus: "serial identity normalization"}
}

func (m SerialModule) Key() string {
	return m.key
}

func (m SerialModule) Description() string {
	return m.focus
}

func (m SerialModule) Priority() int {
	return m.priority
}

func (m SerialModule) Family() string {
	return m.family
}

func (m SerialModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m SerialModule) Validate(input string) error {
...
func (m SerialModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m SerialModule) Transition(stage string) bool {
...
func (m SerialModule) Evidence(input string, now time.Time) Evidence {
...
func (m SerialModule) Score(input string) int {
	value := strings.TrimSpace(input)
