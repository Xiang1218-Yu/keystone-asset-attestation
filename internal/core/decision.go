...
func NewDecisionModule() Module {
	return DecisionModule{key: "decision", priority: 26, family: "asset", focus: "independent decision recording"}
}

func (m DecisionModule) Key() string {
	return m.key
}

func (m DecisionModule) Description() string {
	return m.focus
}

func (m DecisionModule) Priority() int {
	return m.priority
}

func (m DecisionModule) Family() string {
	return m.family
}

func (m DecisionModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m DecisionModule) Validate(input string) error {
...
func (m DecisionModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m DecisionModule) Transition(stage string) bool {
...
func (m DecisionModule) Evidence(input string, now time.Time) Evidence {
...
func (m DecisionModule) Score(input string) int {
	value := strings.TrimSpace(input)
