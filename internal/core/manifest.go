...
func NewManifestModule() Module {
	return ManifestModule{key: "manifest", priority: 44, family: "asset", focus: "evidence manifest assembly"}
}

func (m ManifestModule) Key() string {
	return m.key
}
...
func (m ManifestModule) Priority() int {
	return m.priority
}

func (m ManifestModule) Family() string {
	return m.family
}

func (m ManifestModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m ManifestModule) Validate(input string) error {
...
func (m ManifestModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m ManifestModule) Transition(stage string) bool {
...
func (m ManifestModule) Evidence(input string, now time.Time) Evidence {
...
func (m ManifestModule) Score(input string) int {
	value := strings.TrimSpace(input)
