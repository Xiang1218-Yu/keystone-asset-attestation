...
func NewPolicyModule() Module {
	return PolicyModule{key: "policy", priority: 58, family: "asset", focus: "assurance policy constraints"}
}

func (m PolicyModule) Key() string {
	return m.key
}

func (m PolicyModule) Description() string {
	return m.focus
}
...
func (m PolicyModule) Family() string {
	return m.family
}

func (m PolicyModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m PolicyModule) Validate(input string) error {
...
func (m PolicyModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m PolicyModule) Transition(stage string) bool {
...
func (m PolicyModule) Evidence(input string, now time.Time) Evidence {
...
func (m PolicyModule) Score(input string) int {
	value := strings.TrimSpace(input)
