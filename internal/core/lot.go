...
func NewLotModule() Module {
	return LotModule{key: "lot", priority: 13, family: "asset", focus: "manufacturing lot traceability"}
}

func (m LotModule) Key() string {
	return m.key
}

func (m LotModule) Description() string {
	return m.focus
}

func (m LotModule) Priority() int {
	return m.priority
}

func (m LotModule) Family() string {
	return m.family
}

func (m LotModule) Enabled() bool {
	return m.priority > 0 && strings.TrimSpace(m.key) != ""
}

func (m LotModule) Validate(input string) error {
...
func (m LotModule) Rewrite(input string) string {
	value := strings.TrimSpace(input)
...
func (m LotModule) Transition(stage string) bool {
...
func (m LotModule) Evidence(input string, now time.Time) Evidence {
...
func (m LotModule) Score(input string) int {
	value := strings.TrimSpace(input)
