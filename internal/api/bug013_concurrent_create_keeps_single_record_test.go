...
// POST /v1/records
func TestBug013ConcurrentCreateKeepsSingleRecord(t *testing.T) {
	handler := bugHandler()
	const workers = 20
	...
	for i := 0; i < workers; i++ {
		go func() {
			<-start
			rec := bugJSONRequest(t, handler, http.MethodPost, "/v1/records", map[string]string{"id": "same", "payload": "asset supplier lineage inspection baseline"})
			results <- rec.Code
		}()
	}
	close(start)
	...
	if success != 1 {
		t.Fatalf("concurrent create successes = %d, want 1", success)
	}
}

func TestBug013SingleCreateRegression(t *testing.T) {
	rec := bugJSONRequest(t, bugHandler(), http.MethodPost, "/v1/records", map[string]string{"id": "single", "payload": "asset supplier lineage inspection baseline"})
	if rec.Code != http.StatusCreated {
		t.Fatalf("single create status = %d, want 201", rec.Code)
	}
}
