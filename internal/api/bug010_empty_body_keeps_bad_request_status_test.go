...
// POST /v1/validate
func TestBug010EmptyBodyKeepsBadRequestStatus(t *testing.T) {
	rec := bugRequest(t, bugHandler(), http.MethodPost, "/v1/validate", "")
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("empty body status = %d, want 400", rec.Code)
	}
}

func TestBug010ValidBodyRegression(t *testing.T) {
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("valid body status = %d, want 200", rec.Code)
	}
}
